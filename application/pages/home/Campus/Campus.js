...
Page({
  ...
  onPullDownRefresh: function () {
    ...
    wx.cloud.database().collection('activity_information').get({
      success:res=>{
        console.log("Campus调用get函数成功",res)
        this.setData({
          information:res.data
        })
      },
      fail:res=>{
        console.log("Campus调用get函数失败",res)
      }
    })
  },
  ...
  clickActivity(e){
    ...
    wx.navigateTo({
      url:'./CampusDetail/CampusDetail?item_index='+item_index+"&docID="+docID
    })
  },
  clickSchedule(e) {//添加日程
    ...
    wx.cloud.callFunction({ //云端更新
      ...
      success: res => {
        console.log("Campus调用UpdateUserList-add云函数成功", res)
      },
      fail: res => {
        console.log("Campus调用UpdateUserList-add云函数失败", res)
      }
    })
  },
})