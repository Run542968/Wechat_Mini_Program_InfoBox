...
Page({
  ...
  onPullDownRefresh: function () {
    wx.cloud.database().collection('activity_information').get({
      success:res=>{
        console.log("Lecture调用get函数成功",res)
        this.setData({
          information:res.data
        })
      },
      fail:res=>{
        console.log("Lecture调用get函数失败",res)
      }
    })
  },
  ...
  clickLecture(e){
    ...
    wx.navigateTo({
      url:'./LectureDetail/LectureDetail?item_index='+item_index+"&docID="+docID
    })
  },
  ...
  clickShowDetail(e){
    ...
    if(indexShow.indexOf(index) == -1){
      indexShow.push(index)
      this.setData({indexShow:indexShow})
    }
  },
  remove(array,val){
