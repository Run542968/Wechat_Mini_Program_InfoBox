...
Page({
  ...
  onPullDownRefresh: function () {
    wx.cloud.database().collection('activity_information').doc(this.data.DBdoc).get({
      success:res=>{
        console.log("LectureDetail调用get函数成功",res)
        this.setData({
      ...
      fail:res=>{
        console.log("LectureDetail调用get函数失败",res)
      }
    })
  },
  //-----------自定义函数----------
  ...
  clickSchedule(e) {
    ...
    wx.cloud.callFunction({ //云端更新
      ...
      success: res => {
        console.log("LectureDetail调用UpdateUserList-add云函数成功", res)
      },
      fail: res => {
        console.log("LectureDetail调用UpdateUserList-add云函数失败", res)
      }
    })
  }
})